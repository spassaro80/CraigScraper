    ...
    while i <= len(combined_list_search_terms)-1:
        ...
        for j in range(count):
            if match[j].text is not None:
                products['urls[i]'].append(match[j].text)
        i = i + 1

    dictionary = {'foo' : 'bar'}
    dictionary['foobar'] = 'foobar'
    dictionary['bar'] = ['barfoo']
    dictionary['bar'].append('ffobar')

    context = {'input1' : list_1, 'input2' : list_2, 'input3': input3, 'result1': result1, 'links': urls,  'result3': result3, 'result4': result4, 'products': products, 'dictionary' : dictionary} 
